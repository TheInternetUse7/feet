...
const todoToe: ToeModule = {
  ...
  async execute(message: Message, ctx: ToeContext, args: string[]) {
    ...
    if (sub === 'add') {
      ...
      const item = service.addTask(ctx.db, message.author.id, message.guildId, task);
      await message.reply(`Task #${item.id} added: **${item.task}**`);
      return;
    }

    if (sub === 'list') {
      const items = service.listPending(ctx.db, message.author.id, message.guildId);
      if (items.length === 0) {
    ...
    if (sub === 'clear') {
      const count = service.clearCompleted(ctx.db, message.author.id, message.guildId);
