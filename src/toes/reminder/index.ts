  ...
  async execute(message: Message, ctx: ToeContext, args: string[]) {
    ...
    if (sub === 'in') {
      ...
      if (!durationStr || msgParts.length === 0) {
        await message.reply('Usage: `.remind in <duration> <message>` (e.g., `.remind in 10m Check oven`)');
        return;
      }

      const ms = parseDuration(durationStr);
      if (ms === null) {
        await message.reply('Invalid duration. Use `Ns`, `Nm`, `Nh`, or `Nd` (e.g., `10m`, `2h`, `1d`).');
        return;
      }
      ...
      await message.reply(`Reminder #${item.id} set for <t:${Math.floor(triggerAt.getTime() / 1000)}:R>: **${item.message}**`);
      return;
    }
    ...
    if (sub === 'list') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(0xfee75c)
        .setDescription(
          items.map((i) => `\`#${i.id}\` <t:${Math.floor(new Date(i.trigger_at).getTime() / 1000)}:R> — ${i.message}`).join('\n'),
        );
